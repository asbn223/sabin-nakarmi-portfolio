...
function HelixBackground() {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i <= STEPS; i += 6) {
            ...
            const d1 = new THREE.Mesh(dotGeo, new THREE.MeshBasicMaterial({ color: 0x38bdf8 }));
            d1.position.copy(points1[i]);
            helixGroup.add(d1);
            const d2 = new THREE.Mesh(dotGeo, new THREE.MeshBasicMaterial({ color: 0x818cf8 }));
            d2.position.copy(points2[i]);
            helixGroup.add(d2);
        }
        ...
        for (let i = 0; i < 300; i++) {
            ...
            ambientPos[i * 3 + 1] = (Math.random() - 0.5) * 20;
        ...
        const animate = () => {
            ...
            helixGroup.rotation.y = t * 0.18 + mouse.x * 0.15;
            helixGroup.rotation.x = mouse.y * 0.08;
    ...
    useEffect(() => {
        ...
        const update = () => {
            ...
            const progress = Math.min(
                1,
                Math.max(0, (-rect.top + window.innerHeight * 0.25) / (rect.height - window.innerHeight * 0.25))
            );
...
function ExperienceCard({ exp, index }) {
    ...
    return (
        <div ref={cardRef} className="relative pl-10 group">
            <div
                className="absolute left-0 top-[1.65rem] h-px w-9 opacity-0 group-hover:opacity-100 transition-all duration-500"
                style={{
                    background: 'linear-gradient(to right, #38bdf8cc, transparent)',
                    boxShadow: '0 0 6px 1px #38bdf877',
                }}
            />
            ...
            >
                ...
                <div
                    ...
                    style={{
                        background: 'linear-gradient(to right, transparent 0%, #38bdf844 30%, #818cf844 70%, transparent 100%)',
                    }}
                />
                <div
                    className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none"
                    style={{
                        background: 'radial-gradient(ellipse at 0% 50%, rgba(56,189,248,0.07) 0%, transparent 65%)',
                    }}
                />

                <div className="relative p-6">
                    <div className="flex items-center justify-between mb-3">
            <span className="text-[11px] font-mono tracking-[0.15em] uppercase" style={{ color: '#7dd3fc' }}>
              {exp.period}
            </span>
                        <span
                            className="text-[11px] font-mono tabular-nums opacity-20 group-hover:opacity-60 transition-opacity duration-400"
                            style={{ color: '#818cf8' }}
                        >
            ...
            <span
                className="block h-px w-4 flex-shrink-0 rounded-full opacity-70"
            ...
            />
                    ...
                    <p className="text-[13px] leading-relaxed mb-5 text-slate-400">{exp.description}</p>
